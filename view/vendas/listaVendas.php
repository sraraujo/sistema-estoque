<?php
    $quantidadeRegistros = count($vendas); $dinheiro = 0; $pix = 0; $cartao = 0; $prazo = 0; $vale = 0; $aux = 0;
?>

<div class="container-fluid px-4">
    <div class="row d-flex justify-content-betwee align-items-center">
        <h1 class="mt-4 col-6"> <?= $nomePagina; ?> </h1>
        <div class="col-6 text-end"> <a href="?vendas=novaVenda" class="btn btn-primary mt-4 text-end"> Nova Venda </a> </div>
    </div>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($vendas as $key => $linha) :
                                ...
                                switch (trim($venda[3])) {
                                    ...
                                    case "Cartão";
                                        $cartao += ($venda[1] * $venda[2]);
                                        break;

                                    case "Prazo";
                                        $prazo += ($venda[1] * $venda[2]);
                                        break;

                                    case "Vale";
                                        $vale += ($venda[1] * $venda[2]);
                                        break;
                                }
                            ...
                            ?>
                                ...
                                <tr>
                                    <td> <?= ($key + 1) ?> </td>
                                    <td> <?= $venda[0]; ?> </td>
                                    <td> R$ <?= number_format($venda[1], 2, ",", "."); ?> </td>
                                    <td> <?= $venda[2]; ?> </td>
                                    <td> R$ <?= number_format($venda[1] * $venda[2], 2, ",", "."); ?> </td>
                                    <td> <?= $venda[3]; ?> </td>
                                    <td> <?= $venda[4]; ?> </td>
                                    <td> <?= (isset($venda[5]) ? $venda[5] : "Adminsitrador" ); ?> </td>
                                    <!-- ações -->
                                    <td>
                                        <div class="btn-sm-group" role="group">
                                            <!-- Editar venda -->
                                            <a href="?vendas=editarVenda&token=<?= $token ?>&id=<?= $key ?>" class="btn btn-primary" title="Editar">
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            Tem certeza que deseja excluir o registro de <strong><?= $venda[0]; ?></strong>?
                                                        </div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"> Não </button>
                                                            <input type="hidden" name="vendas" value="deletar">
                                                            <input type="hidden" name="id" value="<?= $key ?>">
                                                            <input type="hidden" name="usuario" value="<?=$_SESSION["user"]["nome"]?>">
                                                            <input type="hidden" name="nome" value="<?= $venda[0]; ?>">
